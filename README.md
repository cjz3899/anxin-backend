...
| `docs/事务消息改造记录.md`           | 任务投递改为事务消息的过程、三个硬约束与踩坑清单          |
| `docs/异步链路遗留问题与修复方案.md` | 状态机三处兜底的修复记录（卡 PROCESSING、reanalyze 状态不一致、重投无容错） |
...
| `10019` | `ANALYSIS_TASK_CREATE_FAILED` | 分析任务创建失败，请稍后重试                   |
...
| OSS 存储失败           | `文件保存失败`（`code=10010`）                                      |
| 任务落库或投递失败     | `分析任务创建失败，请稍后重试`（`code=10019`）                      |
...
- 投递方式：**事务消息**。半消息先落盘，`document` 与 `analysis_task` 两条落库成功后才提交消息；落库失败则消息回滚、Broker
  不可用则本地事务根本不执行，因此不会出现「库里有任务但消息已丢」的中间态。生产者崩溃导致提交状态没送达 broker 时，由 broker
  按消息里的 `taskId` 回查本地事务结果（查到任务补提交，查不到回滚）；
- 主键：`document.id` 与 `analysis_task.id` 改为应用侧雪花算法生成（`SnowUtil`）。原因见上——回查要靠消息体里的 `taskId`
  核对，而消息体在发半消息那一刻就定死了，自增主键来不及；
...
- 僵死回收：消费端抢占 `PROCESSING` 后若写回终态时再次失败，任务会停在中间态。补偿调度器会把 `started_time` 超过 10 分钟
  仍未推进的 `PROCESSING` 任务置回 `PENDING` 并计入一次重试，次数用尽则置 `FAILED` 并同步置文件失败，避免客户端无限等待；
- 补偿扫描按批处理：单轮最多 200 条，且单条投递失败只跳过它自己，不中断本轮其余任务；
...
- 业务说明：对已有文件重新发起分析，创建 **新的** `analysis_task` 并投递消息；消费端会先按文件维度清理旧结果再重新写入。
  适用于分析失败（`FAILED`）后重试——自动重试已穷尽，需用户主动触发。文件状态会与新建任务在同一事务里一并置回 `PENDING`，
  因此列表页与详情页状态一致；若投递失败则两者都不变。
...
自 0.1.6 起，`document.id` 与 `analysis_task.id` 改由应用侧雪花算法生成（`SnowUtil`，19 位），其余表仍用数据库自增。两者都落在
`BIGINT` 范围内，对外接口一律以字符串返回（`documentId`、`taskId`、`id` 等），客户端无需区分。

> 注意：游标分页（§6.5）的「不漏行」依赖「ID 数值序 == 行可见序」。雪花 ID 在 INSERT 之前生成（回查需要消息体提前带上
> `taskId`），并发下这个保证比自增 ID 弱：先生成 ID 的请求可能后提交。因此代码把 ID 生成放在紧贴投递的位置，缩短
> 「生成 ID → 落库可见」的窗口；单实例低并发下实际影响可忽略。
...
| 0.1.6 | 2026-09-18 | 任务创建改走 RocketMQ 事务消息：半消息 + 本地事务（`document`/`analysis_task` 同批落库）+ broker 回查，消除「任务已落库但消息丢失」导致的永久 PENDING；`document` 与 `analysis_task` 主键改为应用侧雪花生成（`SnowUtil`），`IdType` 由 `AUTO` 改 `INPUT`；错误码补充 10019 |
| 0.1.7 | 2026-09-18 | 补齐状态机兜底：补偿调度器新增 `PROCESSING` 超时回收（超 10 分钟置回 `PENDING` 并计数，用尽则置 `FAILED`，避免任务卡在中间态）；重投循环单条失败不再中断整轮、单轮扫描限 200 条；`reanalyze` 同步把 `document.status` 置回 `PENDING`，消除列表与详情状态不一致；重试上限提为共享常量 `TaskConstant.MAX_RETRY` |
